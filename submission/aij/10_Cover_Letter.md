...
2. **Complete Nine-Checkpoint Trajectory**: Across nine empirically evaluated checkpoints ($t \in \{0, 32, 64, 96, 128, 160, 192, 224, 256\}$), the interaction was consistent with a non-decreasing trajectory under prespecified order-restricted analysis despite local point-estimate variation, and was statistically detectable by the earliest available checkpoint ($t=32, \Gamma_{32}=+0.0333$, multiplicity-adjusted 95% CI $[+0.0011, +0.0655]$).
...
Address: Pune, Maharashtra, India (Postal Code / Street: REQUIRED_FROM_AUTHOR)  
